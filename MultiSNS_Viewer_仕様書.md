...
| Phase 7 | 虎威AI下書き統合 | TBD | 全員 | BYOK Gemini下書き＋虎威サブスクゲート |
...
### Phase 7　虎威AI下書き統合（収益モデル連携）

**目標**
虎威サブスクリプションの会員特典として、キーワードから AI 投稿下書きを生成し、本アプリの各SNSへ投稿する導線を提供する。アプリ本体（ビューア）は無料で「認知」を担い、AI 下書きを会員限定機能として「成約」につなげるフリーミアム構成。

**方針（A案：BYOK + status トークン）**
- AI 生成は **BYOK（ユーザー自身の Gemini API キー）でローカル実行** → 虎威の推論コスト 0。
- ゲートは虎威への **subscription_status のみ**の問い合わせ（虎威発行の status トークン → Function → `{active}`）。Electron 側 Google OAuth は実装しない。
- 既存 `snake-sns` の生成ロジック（`utils/AI/postApi.ts`：Gemini・48件・JSON整形）を移植。
- 詳細設計・API契約・タスク分解は **`docs/phase7-torai-ai.md`** を参照。

**主なタスク**
- status トークン登録UI ＋ 虎威 `/verifyStatus` 連携（再検証サイクル・オフライン猶予・失効UI）
- Gemini キー登録（safeStorage）＋ main での生成（IPC、サービス別プロンプト）
- 下書きUI（キーワード→48件→編集/選択→アクティブカラムの compose へ）
- `@google/generative-ai` 依存追加（ネイティブ依存なし＝MSIX/公証に影響なし）

**完了基準**
- active 会員のみ AI 下書きが利用可能、非アクティブ/失効時は機能無効化＋虎威セールス導線
- キーワードから下書き生成→選択→各SNSの投稿フローへ受け渡しできる
- Gemini キー・status トークンが safeStorage に暗号化保存され、ログに露出しない

**要決定事項（虎威・プロダクト側）**
- status トークンの寿命/失効方式、再検証サイクル＋オフライン猶予日数、v1プロンプトのSNS対応範囲、レート制限/エラーコード体系

**スコープ外**
- node-llama-cpp ローカルLLM（将来「キー不要・オフライン」版として再検討）／虎威サーバ生成（コスト負担のため不採用）／アプリ内 OAuth（A案では不要）
